...
from flask import render_template, url_for, flash, redirect, request, Blueprint, Response, current_app, session, \
...
def account():
    ...
    print(projects[0].user_id, current_user.id, 'test 130')
...
def project(project_id):
    ...
    print('test423', project_id)
    project_name = Project.query.filter(Project.id == project_id).first().project_name

    if form.validate_on_submit():  # change the project name and change the name for the related users in this project
        print("I am here 33")
        ...
        try:
            ...
            if new_member_name:
                try:  # add new member
                    ...
                    existing_member_of_this_project = Projectuser.query.filter(
                        Projectuser.user_id == new_member_user_id, Projectuser.project_id == project_id).first()
                    if not existing_member_of_this_project:
                        project_name = Project.query.filter(Project.id == project_id).first().project_name
                        projectuser = Projectuser(project_name=project_name, user_id=new_member_user_id,
                                                  permission="view", project_id=project_id)
                ...
                Projectuser.query.filter(Projectuser.user_id == remove_member_id,
                                         Projectuser.project_id == project_id).delete()
                db.session.commit()
                ...
                print(dummy_user_id)
                if not Annotation.query.filter(Annotation.doc_id == annotated_doc_id,
                    ...
                    for i in range(len(Sent.query.filter(Sent.doc_id == annotated_doc_id).all())):
                        ...
                        dummy_annotation = Annotation.query.filter(Annotation.doc_id == annotated_doc_id,
                                                                   Annotation.sent_id == i + 1,
                                                                   Annotation.user_id == dummy_user_id).first()
                        if dummy_annotation:
                            print('I am here 35')
                            annotation = Annotation(sent_annot=dummy_annotation.sent_annot,
                                                    doc_annot=dummy_annotation.doc_annot,
                                                    alignment=dummy_annotation.alignment, author=current_user,
                                                    sent_id=dummy_annotation.sent_id, doc_id=dummy_annotation.doc_id,
                                                    sent_umr=dummy_annotation.sent_umr,
                                                    doc_umr=dummy_annotation.doc_umr)
                ...
                print("I am here 34")
                Annotation.query.filter(Annotation.user_id == current_user.id,
                                        Annotation.doc_id == delete_annot_doc_id).delete()
                logging.info(db.session.commit())
                ...
                if not (Annotation.query.filter(Annotation.doc_id == add_qc_doc_id, Annotation.user_id == qc_id).all()):
                    print("I am here 70")
                    member_annotations = Annotation.query.filter(Annotation.doc_id == add_qc_doc_id,
                                                                 Annotation.user_id == current_user.id).all()
                    for a in member_annotations:
                        qc_annotation = Annotation(sent_annot=a.sent_annot, doc_annot=a.doc_annot,
                                                   alignment=a.alignment, author=qc,
                                                   sent_id=a.sent_id, doc_id=a.doc_id, sent_umr=a.sent_umr,
                                                   doc_umr=a.doc_umr)
                        db.session.add(qc_annotation)
                    docqc = Docqc(doc_id=add_qc_doc_id, project_id=project_id,
                                  author=current_user)  # document which member uploaded the qc doc of this project
                ...
                # check existing:
                if not Docda.query.filter(Docda.project_id == project_id, Docda.user_id == current_user.id,
                                          Docda.doc_id == add_da_doc_id).all():
                    ...
                    docqc = Docqc.query.filter(Docqc.project_id == project_id, Docqc.doc_id == add_da_doc_id,
                                               Docqc.upload_member_id != current_user.id).first()
                    print("docqc: ", docqc)
                    if docqc:  # if there is a qc version of this doc already
                        if not Docda.query.filter(Docda.project_id == project_id,
                                                  Docda.user_id == docqc.upload_member_id,
                            ...
                            docda_qc = Docda(project_id=project_id, user_id=docqc.upload_member_id,
                                             doc_id=add_da_doc_id)
                            db.session.add(docda_qc)
                    logging.info(db.session.commit())
            elif rm_da_doc_id != 0 and rm_da_user_id != 0:
                print("I am here 66")
                Docda.query.filter(Docda.project_id == project_id, Docda.user_id == rm_da_user_id,
                                   Docda.doc_id == rm_da_doc_id).delete()
                logging.info(db.session.commit())
        except Exception as e:
            print(e)
    ...
    checked_out_by = []
    qcAnnotations = Annotation.query.filter(Annotation.user_id == Project.query.get(int(project_id)).qc_user_id,
                                            Annotation.sent_id == 1).all()  # when add to my annotation, anntation row of sent1 got added in Annotation table, therefore check if there is annotation for sent1
    qcDocs = []
    qcUploaders = []
    qcUploaderIds = []
    for qca in qcAnnotations:
        qcDocs.append(Doc.query.filter(Doc.id == qca.doc_id).first())
        uploader_id = Docqc.query.filter(Docqc.doc_id == qca.doc_id,
                                         Docqc.project_id == project_id).first().upload_member_id
    ...
    return render_template('project.html', title='project', project_name=project_name, project_id=project_id,
                           ...
                           checked_out_by=list(checked_out_by),
                           projectDocs=projectDocs, qcDocs=qcDocs, qcUploaders=qcUploaders, qcUploaderIds=qcUploaderIds,
...
def partialgraph(project_id):
    ...
    return render_template('partial_graph.html', title='partial graphs', partialGraphs=partialGraphs,
...
def alllexicon(project_id):
    ...
    if request.method == "POST":
        ...
        if deleteLemmaKey:
            ...
            try:
                share2projectId = Projectuser.query.filter(Projectuser.project_name == share2projectName,
                                                           Projectuser.user_id == current_user.id).first().project_id
            ...
            try:
                ...
                return make_response(jsonify(
                    {"msg": "delete whole lexicon successfully, refresh to see changes", "msg_category": "success"}),
                    200)  # need refresh to see is because, redirect to current page, under post somehow doesn't work, therefore I will have to ask the user to refresh manually
            except Exception as e:
                print(e)
                print("delete lexicon error")

    all_projects = Projectuser.query.filter(Projectuser.user_id == current_user.id,
                                            Projectuser.permission == "admin").all()
    all_project_names = [p.project_name for p in all_projects]
    return render_template('alllexicon.html', title='all lexicon', lexi=json.dumps(lexi), project_name=project_name,
                           project_id=project_id, all_projects=json.dumps(all_project_names))
...
def discourse(project_id):
    ...
    return render_template('discourse.html', project_id=project_id, current_setting=json.dumps(
        Lattice.query.filter(Lattice.project_id == project_id).first().discourse))
...
def aspect(project_id):
    ...
    return render_template('aspect.html', project_id=project_id, current_setting=json.dumps(
        Lattice.query.filter(Lattice.project_id == project_id).first().aspect))
...
def person(project_id):
    ...
    return render_template('person.html', project_id=project_id, current_setting=json.dumps(
        Lattice.query.filter(Lattice.project_id == project_id).first().person))
...
def number(project_id):
    ...
    return render_template('number.html', project_id=project_id, current_setting=json.dumps(
        Lattice.query.filter(Lattice.project_id == project_id).first().number))
...
def modal(project_id):
    ...
    if request.method == 'POST':
        try:
            ...
            print(e)
            print("get modal setting error")
    return render_template('modal.html', project_id=project_id, current_setting=json.dumps(
        Lattice.query.filter(Lattice.project_id == project_id).first().modal))


@users.route('/modification/<int:project_id>', methods=['GET', 'POST'])
def modification(
        project_id):  # there is no post here like the previous ones, because users are not allowed to toggle off any modification items in the dropdown menu
    if not current_user.is_authenticated:
        return redirect(url_for('users.login'))
