            ...
            for form_file in form.files.data:
                ...
                if is_exported:  # has annotation
                    if file_format == 'isi_editor':
                        new_content_string, sents, sent_annots, doc_annots = process_exported_file_isi_editor(
                            content_string)
...
@main.route("/doclevel_thyme_sent/<string:doc_sent_id>", methods=['GET', 'POST'])
def doclevel_thyme_sent(doc_sent_id):
    if not current_user.is_authenticated:
        return redirect(url_for('users.login'))
    doc_id = int(doc_sent_id.split("_")[0])
    default_sent_id = int(doc_sent_id.split("_")[1])
