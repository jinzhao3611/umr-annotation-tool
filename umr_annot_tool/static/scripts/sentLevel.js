...
function string2amr_rec(annotText, loc, state, ht) {
    ...
    if (state === 'pre-open-para') { //todo
        ...
        if (annotText.match(/^\(\s*[a-zA-Z0-9][-_a-zA-Z0-9']*(\s*\/\s*|\s+)[:*]?[a-zA-Z0-9][-_a-zA-Z0-9']*[*]?[\s)]/) // match something like (s1s / shadahast
            ...
            let variableList = annotText.match(/^[a-zA-Z0-9][-_a-zA-Z0-9']*/);
            ...
            if(language === "chinese"){
                ...
                conceptList = annotText.match(/^[:*]?[a-zA-Z0-9][-_a-zA-Z0-9']*[*]?/);
            ...
            let new_variable, decorated_variable, new_concept, decorated_concept;
            ...
            annotText = string2amr_rec(annotText, loc, 'post-concept', ht); // (
        ...
        if (role_follows_p || open_para_follows_p) {
            ...
            if (annotText.match(/^\s*\(/)) { // )
                annotText = string2amr_rec(annotText, new_loc, 'pre-open-para', ht);
            } else {
                annotText = string2amr_rec(annotText, new_loc, 'post-role', ht);
            }
            annotText = string2amr_rec(annotText, loc, 'post-concept', ht);
        }
        ...
        var head_loc = loc.replace(/\.\d+$/, "");
        annotText = string2amr_rec(annotText, head_loc, 'post-concept', ht);
    }
    return annotText;
}
