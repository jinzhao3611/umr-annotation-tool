...
function addDocToProj(doc_id, project_id){
    ...
    fetch(`/project/${project_id}`, {
        method: 'POST',
        body: JSON.stringify({"update_doc_id": doc_id, "update_doc_project_id": project_id, "new_member_name": "", "remove_member_id": 0})
    }).catch(function(error){
        console.log("Fetch error: "+ error);
    });
}
...
function removeDocFromProj(doc_id, project_id){
    ...
    fetch(`/project/${project_id}`, {
        method: 'POST',
        body: JSON.stringify({"update_doc_id": doc_id, "update_doc_project_id": 0, "new_member_name": "", "remove_member_id": 0})
    }).catch(function(error){
        console.log("Fetch error: "+ error);
    });
}
...
function addNewMember(project_id){
    ...
    fetch(`/project/${project_id}`, {
        method: 'POST',
        body: JSON.stringify({"new_member_name": member_name, "update_doc_id": 0, "update_doc_project_id": 0, "remove_member_id":0})
    }).then(function(response){
        return response.json()
    }).then(function(data){
...
//    to remove member: 1. remove the row that contains current project and current user
    if (confirm("Are you sure you want to remove this user from this project?")) {
        ...
        fetch(`/project/${project_id}`, {
            method: 'POST',
            body: JSON.stringify({"remove_member_id": member_id, "new_member_name": "", "update_doc_id": 0, "update_doc_project_id": 0})
        }).catch(function(error){
            console.log("Fetch error: "+ error);
        });
    } else {
