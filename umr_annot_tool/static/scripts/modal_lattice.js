console.log('modal_lattice.js loaded');

function initModalLattice() {
    console.log('Initializing modal strength lattice...');
    
    // Define the data structure for the modal strength lattice based on the image
    const data = {
        name: "Modal Strength",
        children: [
            {
                name: "Affirmative Modal Strength",
                children: [
                    {
                        name: "Non-Neutral",
                        children: [
                        ...
                        children: [
                            {
                                name: "Partial",
                                children: [
                                    { name: "Strong Partial", children: [] },
                                    { name: "Weak Partial", children: [] }
                                ]
                            },
                            {
                                name: "Neutral",
                                children: [
                                    { name: "Strong Neutral", children: [] },
                                    { name: "Weak Neutral", children: [] }
                                ]
                            }
                        ]
                    }
    ...
    let currentSetting = {};
    try {
        const currentSettingElement = document.getElementById('modal-current-setting');
    ...
    const margin = { top: 50, right: 350, bottom: 50, left: 150 };
    const width = 960 - margin.left - margin.right;
    const height = 600 - margin.top - margin.bottom;

    // Remove any existing SVG
    d3.select("#modal-lattice-container svg").remove();

    // Create the SVG container
    const svg = d3.select("#modal-lattice-container")
        .append("svg")
    ...
    const toggles = nodes.filter(d => d.data.name !== "Modal Strength")
