console.log('number_lattice.js loaded');

function initNumberLattice() {
    console.log('Initializing number lattice...');
    
    // Define the data structure for the number lattice based on the image
    const data = {
        name: "Number",
        children: [
            {
    ...
    let currentSetting = {};
    try {
        const currentSettingElement = document.getElementById('number-current-setting');
    ...
    d3.select("#number-lattice-container svg").remove();
    
    // Create the SVG container
    const svg = d3.select("#number-lattice-container")
        .append("svg")
    ...
    function saveSettings(settings) {
        ...
        .then(data => {
            ...
            if (data.msg_category === 'success') {
                ...
                const errorMsg = document.getElementById('number-error-msg');
    ...
    const toggles = nodes.filter(d => d.data.name !== "Number")
