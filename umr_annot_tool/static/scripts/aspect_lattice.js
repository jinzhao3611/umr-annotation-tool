console.log('aspect_lattice.js loaded');

function initAspectLattice() {
    console.log('Initializing aspect lattice...');
    
    // Define the data structure for the aspect lattice based on the image
    const data = {
        name: "Aspect",
        children: [
            { name: "Habitual", children: [] },
            { name: "Imperfective", children: [] },
            {
                name: "Process",
                children: [
                    {
                        ...
                        children: [
                            {
                                name: "State",
                                children: [
                                    { name: "Reversible State", children: [] },
                                    { name: "Irreversible State", children: [] },
                                    { name: "Point State", children: [] },
                                    { name: "Inherent State", children: [] }
                                ]
                            },
                            {
                                name: "Activity",
                                children: [
                                    { name: "Directed Activity", children: [] },
                                    { name: "Undirected Activity", children: [] }
                                ]
                            }
                        ]
                    },
                        ...
                        children: [
                            {
                                name: "Endeavor",
                                children: [
                                    { name: "Semelfactive", children: [] },
                                    { name: "Undirected Endeavor", children: [] },
                                    { name: "Directed Endeavor", children: [] }
                                ]
                            },
                            {
                                name: "Performance",
                                children: [
                                    { name: "Incremental Accomplishment", children: [] },
                                    { name: "Non-incremental Accomplishment", children: [] },
                                    {
                                        name: "Directed Achievement",
                                        children: [
                                            { name: "Reversible Directed Achievement", children: [] },
                                            { name: "Irreversible Directed Achievement", children: [] }
                                        ]
                                    }
                                ]
                            }
    ...
    let currentSetting = {};
    try {
        const currentSettingElement = document.getElementById('aspect-current-setting');
    ...
    d3.select("#aspect-lattice-container svg").remove();

    // Create the SVG container
    const svg = d3.select("#aspect-lattice-container")
        .append("svg")
    ...
    function saveSettings(settings) {
        ...
        .then(data => {
            ...
            if (data.msg_category === 'success') {
                ...
                const errorMsg = document.getElementById('aspect-error-msg');
    ...
    const toggles = nodes.filter(d => d.data.name !== "Aspect")
